...
diagnose <- function(series, order, seasonal = c(0,0,0), lambda = NULL,
  ...
  if (!is.null(plot_tag)) {
    fn <- sprintf("figures/im_%s_resid.png", plot_tag)
...
png("figures/im_step0_seasdiff.png", width = 1000, height = 420, res = 110, bg = "white")
...
if (!is.null(tfr_init)) coefs(tfr_init$fit, tfr_init$label)
if (!is.null(tfr_impr)) coefs(tfr_impr$fit, tfr_impr$label)
if (!is.null(tlb_init)) coefs(tlb_init$fit, tlb_init$label)
if (!is.null(tlb_impr)) coefs(tlb_impr$fit, tlb_impr$label)

# =============================================================================
# SUMMARY TABLE — initial vs improved
# =============================================================================
rule("SUMMARY — initial vs improved (AIC / LB p / npar / test error)")
row <- function(series, role, m) if (!is.null(m))
  cat(sprintf("  %-4s %-9s %-30s npar=%2d AIC=%9.2f LBp=%.3f RMSE=%9.3f MAE=%9.3f MAPE=%5.2f\n",
              series, role, m$label, m$np, m$aic, m$lb_p, m$rmse, m$mae, m$mape))
row("TFR","initial", tfr_init); row("TFR","improved", tfr_impr)
row("TLB","initial", tlb_init); row("TLB","improved", tlb_impr)

# =============================================================================
# FORECAST PLOTS for the four selected models
# =============================================================================
rule("FORECAST PLOTS -> figures/im_*_forecast.png")
...
fc_plot(tfr_init, train$Year, train$TFR, "TFR", "figures/im_tfr_initial_forecast.png", paste0("TFR initial — ", tfr_init$label))
fc_plot(tfr_impr, train$Year, train$TFR, "TFR", "figures/im_tfr_improved_forecast.png", paste0("TFR improved — ", tfr_impr$label))
fc_plot(tlb_init, train$Year, train$TLB, "TLB", "figures/im_tlb_initial_forecast.png", paste0("TLB initial — ", tlb_init$label))
fc_plot(tlb_impr, train$Year, train$TLB, "TLB", "figures/im_tlb_improved_forecast.png", paste0("TLB improved — ", tlb_impr$label))
