...
# =============================================================================
# TFR CANDIDATE MODELS (SARIMA)
# =============================================================================
cat("\n=================================================================\n")
cat("TFR CANDIDATE MODELS (SARIMA)\n")
cat("=================================================================\n\n")

tfr_sarima_cands <- list(
  safe_fit(quote(Arima(tfr_train, order=c(0,1,0), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(0,1,0)(1,0,0)[12]"),
  safe_fit(quote(Arima(tfr_train, order=c(0,1,1), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(0,1,1)(1,0,0)[12]"),
  safe_fit(quote(Arima(tfr_train, order=c(1,1,0), seasonal=list(order=c(0,0,1), period=12))), "SARIMA(1,1,0)(0,0,1)[12]"),
  safe_fit(quote(Arima(tfr_train, order=c(1,1,1), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(1,1,1)(1,0,0)[12]"),
  safe_fit(quote(Arima(tfr_train, order=c(1,1,0), seasonal=list(order=c(1,0,1), period=12))), "SARIMA(1,1,0)(1,0,1)[12]"),
  safe_fit(quote(Arima(tfr_train, order=c(2,1,0), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(2,1,0)(1,0,0)[12]"),
  safe_fit(quote(Arima(tfr_train, order=c(1,1,0), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(1,1,0)(1,0,0)[12]")
)
tfr_sarima_cands <- Filter(Negate(is.null), tfr_sarima_cands)
for (m in tfr_sarima_cands) print_row(m)

tfr_viable <- Filter(function(m) m$viable, tfr_sarima_cands)
tfr_best   <- tfr_viable[[which.min(sapply(tfr_viable, function(m) m$aic))]]

# =============================================================================
# TLB CANDIDATE MODELS (SARIMA)
# =============================================================================
cat("\n=================================================================\n")
cat("TLB CANDIDATE MODELS (SARIMA)\n")
cat("=================================================================\n\n")

tlb_sarima_cands <- list(
  safe_fit(quote(Arima(tlb_train, order=c(1,1,0), seasonal=list(order=c(0,0,1), period=12))), "SARIMA(1,1,0)(0,0,1)[12]"),
  safe_fit(quote(Arima(tlb_train, order=c(2,1,0), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(2,1,0)(1,0,0)[12]"),
  safe_fit(quote(Arima(tlb_train, order=c(0,1,1), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(0,1,1)(1,0,0)[12]"),
  safe_fit(quote(Arima(tlb_train, order=c(1,1,0), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(1,1,0)(1,0,0)[12]"),
  safe_fit(quote(Arima(tlb_train, order=c(1,1,1), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(1,1,1)(1,0,0)[12]"),
  safe_fit(quote(Arima(tlb_train, order=c(1,1,0), seasonal=list(order=c(1,0,1), period=12))), "SARIMA(1,1,0)(1,0,1)[12]"),
  safe_fit(quote(Arima(tlb_train, order=c(0,1,0), seasonal=list(order=c(1,0,0), period=12))), "SARIMA(0,1,0)(1,0,0)[12]")
)
tlb_sarima_cands <- Filter(Negate(is.null), tlb_sarima_cands)
for (m in tlb_sarima_cands) print_row(m)

tlb_viable <- Filter(function(m) m$viable, tlb_sarima_cands)
tlb_best   <- tlb_viable[[which.min(sapply(tlb_viable, function(m) m$aic))]]

# =============================================================================
# RESIDUAL DIAGNOSTICS PANELS
# =============================================================================
...
# FORECASTS
# =============================================================================
pred_tfr <- forecast(tfr_best$model, h = n_test, level = c(80, 95))
pred_tlb <- forecast(tlb_best$model, h = n_test, level = c(80, 95))
fc_tfr   <- as.numeric(pred_tfr$mean)
fc_tlb   <- as.numeric(pred_tlb$mean)
lo80_tfr <- as.numeric(pred_tfr$lower[,1]); hi80_tfr <- as.numeric(pred_tfr$upper[,1])
lo95_tfr <- as.numeric(pred_tfr$lower[,2]); hi95_tfr <- as.numeric(pred_tfr$upper[,2])
lo80_tlb <- as.numeric(pred_tlb$lower[,1]); hi80_tlb <- as.numeric(pred_tlb$upper[,1])
lo95_tlb <- as.numeric(pred_tlb$lower[,2]); hi95_tlb <- as.numeric(pred_tlb$upper[,2])
acc_tfr  <- acc(tfr_test, fc_tfr)
acc_tlb  <- acc(tlb_test, fc_tlb)
...
make_fc_plot(tfr_ts, tfr_test, fc_tfr, lo80_tfr, hi80_tfr, lo95_tfr, hi95_tfr,
